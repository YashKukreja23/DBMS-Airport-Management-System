...
if (!$conn) {
    ...
    $sql="SELECT * FROM ticket_price_history;";
  $result = mysqli_query($conn, $sql);
  $resultCheck = mysqli_num_rows($result);

  if($resultCheck){
    while ($row = mysqli_fetch_assoc($result)){
      echo "<tr><td>". $row["DATE_OF_BOOKING"]."</td><td>". $row["SOURCE"]."</td><td>".$row["DESTINATION"]."</td><td>".$row["CLASS"]."</td><td>". $row["PRICE"]."</td><td>".$row["DATE_OF_UPDATE"];
    }
  }
  else{
    echo "0 results";
